...
export const metadata = {
  title: "Template Posts — Easy Mickey",
};
  ...
  return (
    <main className="min-h-screen bg-[#0a0a12]">
      <header
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(236,72,153,0.08) 0%, transparent 70%), #0a0a12",
        }}
      >
        <div className="mx-auto max-w-lg">
          ...
          <h1 className="text-2xl font-bold text-[#f0eff8]">Template Posts</h1>
          <p className="mt-1 text-sm text-[#6a6880]">
            คลิกเพื่อโพสต์บน X พร้อมข้อความสำเร็จรูป
          </p>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-lg space-y-3 px-4 pb-16 pt-4">
        {templates.map((t) => {
          const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(resolveTemplate(t.text))}`;
          return (
            <a
              key={t.id}
              href={url}
        ...
        {templates.length === 0 && (
          <p className="text-center text-sm text-[#6a6880]">ไม่มี template</p>
        )}
      </div>
    </main>
  );
